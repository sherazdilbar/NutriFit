...
export default function LogExercise() {
  ...
  useEffect(() => {
    ...
    // Check if exercise parameter exists in URL
  ...
  const handleSearch = async (term: string) => {
    ...
    try {
      const response = await fetch(`/api/exercises?search=${encodeURIComponent(term)}`);
      const data = await response.json();
      if (data.success) {
  ...
  const updateExercise = (id: number, field: 'sets' | 'reps' | 'duration', value: any) => {
    setSelectedExercises(selectedExercises.map(e => 
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      {showSuccess && (
        <div className="fixed top-20 right-4 bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg z-50">
          <p className="font-medium">Workout logged successfully!</p>
        </div>
      )}
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Page Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-1">Log Your Workout</h1>
          <p className="text-gray-600">Track your exercise routine</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg border border-gray-200 p-6 mb-5">
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Search Exercises</h2>
                  ...
                  {searchResults.map((exercise) => (
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <span className={`inline-flex mt-1 px-2 py-1 rounded-full text-xs font-semibold ${getImpactColor(exercise.impactLevel)}`}>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Selected Exercises</h2>
              
              {selectedExercises.length === 0 ? (
                <p className="text-gray-500 text-center py-8 text-sm">No exercises selected yet. Search and add exercises above.</p>
              ) : (
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        <div>
                          ...
                          <input
                            ...
                            className="w-full px-3 py-2 rounded-lg border border-gray-300 focus:border-primary-500 focus:ring-2 focus:ring-primary-200 focus:outline-none text-center text-sm"
                            min="1"
                          />
                        </div>
                        <div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg border border-gray-200 p-6 sticky top-20">
              ...
              <div className="bg-primary-50 rounded-lg p-4 mb-6 border border-primary-100">
                <p className="text-sm text-primary-700 font-medium mb-1">Total Exercises</p>
                <p className="text-3xl font-bold text-primary-900">{selectedExercises.length}</p>
                <p className="text-sm text-primary-600 mt-1">
                  {selectedExercises.reduce((sum, e) => sum + e.sets, 0)} total sets
                </p>
              </div>
